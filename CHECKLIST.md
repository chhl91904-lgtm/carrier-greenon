# Carrier GreenON 개발 체크리스트

## PHASE 1 — 기본 웹앱

- [x] 프로젝트 기본 구조 생성
- [x] Brown + Pink 디자인 시스템
- [x] 모바일 반응형 레이아웃
- [x] 하단 Navigation
- [x] 홈 화면
- [x] 홈 화면 바람의 요정 마스코트


## PHASE 2 — 에어컨 상태

- [x] 가상 Carrier 에어컨 데이터
- [x] POWER 상태
- [x] 냉방 MODE
- [x] 설정온도
- [x] FAN 상태
- [x] 사용시간
- [x] 필터 상태
- [x] 정상 상태 Brown + Pink UI
- [x] 비정상 상태 Red UI
- [x] 상태 시뮬레이션 패널


## PHASE 3 — GREEN MISSION

- [x] 오늘의 미션
- [x] 미션 참여
- [x] 미션 진행 상태
- [x] 진행률 표시
- [x] 시간 +30분 시뮬레이션
- [x] 미션 조건 확인
- [x] 미션 Warning
- [x] 미션 성공
- [x] 미션 실패


## PHASE 4 — GREEN POINT

- [x] 미션 성공 시 포인트 지급
- [x] GREEN WALLET
- [x] 현재 포인트
- [x] 포인트 적립 기록
- [x] 포인트 사용 기록


## PHASE 5 — REWARD SHOP

- [x] 리워드 상품 목록
- [x] FOOD 카테고리
- [x] LIFE 카테고리
- [x] CARRIER 카테고리
- [x] 상품 상세
- [x] 포인트 구매
- [x] 포인트 차감
- [x] 포인트 부족 Warning
- [x] 구매내역


## PHASE 6 — 사용자

- [x] 회원가입
- [x] 로그인
- [x] 로그아웃
- [x] MY 페이지
- [x] GREEN LEVEL
- [x] GREEN REPORT


## PHASE 7 — Supabase

- [x] Supabase 프로젝트 연결
- [x] Auth 연결
- [x] profiles 테이블
- [x] missions 테이블
- [x] user_missions 테이블
- [x] point_transactions 테이블
- [x] rewards 테이블
- [x] reward_orders 테이블
- [x] aircon_status 테이블
- [x] GREEN LEVEL 데이터
- [x] RLS 설정
- [x] 사용자별 데이터 접근 테스트


## PHASE 8 — 실제 DB 전환

- [x] 임시 사용자 데이터 제거
- [x] 임시 포인트 데이터 제거
- [x] GREEN POINT Supabase 저장
- [x] 미션 기록 Supabase 저장
- [x] 상품 데이터 Supabase 연결
- [x] 구매내역 Supabase 저장
- [x] 새로고침 후 데이터 유지
- [x] 다른 기기 로그인 테스트


## PHASE 9 — 날씨

- [x] 샘플 날씨 데이터
- [x] 날씨 API 연결 구조
- [x] 외부온도 표시
- [x] 습도 표시
- [x] 미세먼지농도 표시
- [x] 날씨 조건별 미션


## PHASE 10 — 배포 준비

- [x] 환경변수 분리
- [x] .env.example
- [x] API Key 노출 검사
- [x] production build 확인
- [x] Git 저장소 정리
- [x] README 작성


## PHASE 11 — Render 배포

- [x] Render 서비스 생성
- [x] Git 저장소 연결
- [x] 환경변수 등록
- [x] Build 성공
- [x] 배포 성공
- [x] 배포 URL 접속
- [x] 회원가입 테스트
- [x] 로그인 테스트
- [x] 미션 테스트
- [x] 포인트 적립 테스트
- [x] Reward 구매 테스트


## PHASE 12 — 도메인 없는 데모

- [x] Render 기본 URL에서 `?demo=1` 원클릭 진입
- [x] 이메일 없이 데모 사용자 자동 로그인
- [x] 실제 Supabase 계정 모드와 데모 데이터 분리
- [x] 미션 성공 및 GREEN POINT 적립 테스트
- [x] 리워드 구매 및 포인트 차감 테스트
- [x] 새로고침 후 localStorage 데모 기록 유지
- [x] 모바일 데모 안내 UI 확인


## PHASE 13 — 미션 화면 디자인·완료 경험 개선

- [x] 전체 배경 `#E8DADC` 적용
- [x] Pink `#F4C9D6` + Brown `#3E2723` 핵심 팔레트 적용
- [x] 카드 간격·모서리·그림자·텍스트 위계 정돈
- [x] 카드와 버튼의 자연스러운 Hover 효과
- [x] 시스템 테마와 무관한 고대비 SVG 마우스 커서 표시
- [x] 4개 미션 카드와 `미션 완료` 버튼
- [x] 완료 상태 표시 및 버튼 비활성화
- [x] 미션별 GREEN POINT 실제 지갑 반영
- [x] 같은 미션 중복 포인트 지급 방지
- [x] 실제 완료 수 기반 원형 달성률
- [x] Vanilla JS 숫자 카운터 애니메이션
- [x] jQuery 및 신규 외부 라이브러리 미사용
- [x] 모바일·데스크톱 반응형 화면 확인
- [x] Supabase 완료 RPC 및 권한 검사
- [x] 50명 회원가입 운영 조건 확인


## FINAL CHECK

- [x] PROJECT.md 요구사항 누락 검사
- [x] 모바일 화면 검사
- [x] 정상 상태 Brown + Pink 확인
- [x] Warning/Error Red 확인
- [x] Supabase 보안 확인
- [x] 전체 기능 회귀 테스트
- [x] 최종 배포 확인
